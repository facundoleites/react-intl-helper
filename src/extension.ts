...
export async function activate(context: vscode.ExtensionContext) {
  ...
  vscode.window.createTreeView("react-intl-helper-tree-view", {
    treeDataProvider: translationsTree,
  });
  // Make sure we register a serializer in activation event
  vscode.window.registerWebviewPanelSerializer(TranslationsPanel.viewType, {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeTextEditorSelection(async (e) => {
      if (e.selections) {
        ...
        const selectedJSONPos = json.getLocation(
          text,
          e.textEditor.document.offsetAt(sel.start)
        );
        ...
        const thisLangKeys = Object.keys(defaultData).map((defaultKey) =>
          typeof thisLangData[defaultKey] === "undefined" ? defaultKey : false
        );
        const missingTranslations = [...thisLangKeys].filter(
          Boolean
        ) as string[];
  ...
  context.subscriptions.push(
    ...
    vscode.window.onDidChangeActiveTextEditor(verifyActiveEditor)
  );
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (vscode.workspace.workspaceFolders) {
        ...
        if (changedRoot || changedDefaultLang) {
          vscode.commands.executeCommand("react-intl-helper.refresh");
        }
      }
    })
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("react-intl-helper.openPanel", () => {
      TranslationsPanel.createOrShow(context.extensionUri);
    })
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("react-intl-helper.refresh", () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("react-intl-helper.openFile", (filepath) =>
